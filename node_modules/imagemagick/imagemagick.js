...
function parseIdentify_misha(input) {
  var lines = input.split("\n"),
      prop = {},
      currentLine;
  ...
  for (i in lines) {
    ...
    if (currentLine.length > 0) {
      //console.log("++++++++++++++++++++");
      var _match = /Geometry: ([\d]+)x([\d]+)/.exec(currentLine);
      //var _depthmatch = /Depth: ([\d]+-bit)/.exec(currentLine);
      //var _qualityhmatch = /Quality: ([\d]+)/.exec(currentLine);
      
      //console.log(currentLine);
      //console.log("++++++++++++++++++++");
      if(_match&&_match.length>=3){
        prop.width = parseInt(_match[1]);
        prop.height = parseInt(_match[2]);
      }/*else if(_depthmatch&&_depthmatch.length>=2){
        prop.depth = parseInt(_depthmatch[1]);
      }else if(_qualityhmatch&&_qualityhmatch.length>=2){
        prop.quality = parseInt(_qualityhmatch[1]);
      }*/
    }
  }
  return prop;
  ...
  var proc = exec2(exports.identify.path, args, {timeout:120000}, function(err, stdout, stderr) {
    ...
    if (!err) {
      ...
      if (isCustom) {
        ...
        result = parseIdentify_misha(stdout);
      }
    }
    callback(err, result);
  });
